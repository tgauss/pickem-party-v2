...
async function settleWeek14() {
  console.log('=== SETTLING WEEK 14 ===\n');

  // Get Week 14 picks with user and game info
  ...
  for (const pick of losingPicks) {
    ...
    if (member.is_eliminated) {
      console.log(`  Already eliminated, skipping (ghost pick)`);
      continue;
    }
  ...
  const { data: eliminated } = await supabase
    ...
    .eq('eliminated_week', 14);

  if (eliminated && eliminated.length > 0) {
    console.log('\nEliminated in Week 14:');
