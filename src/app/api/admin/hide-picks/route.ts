...
export async function POST(request: Request) {
  try {
    ...
    if (!isAuthorized) {
      return NextResponse.json({ 
        success: false, 
        error: 'Unauthorized: Only commissioners and super admins can hide picks' 
      })
    }
    ...
    // Check if week is already hidden
    if (!currentRevealedWeeks.includes(week)) {
      return NextResponse.json({ 
        success: false, 
        error: `Week ${week} picks are already hidden` 
      })
    }

    // Remove the week from revealed weeks
    const updatedRevealedWeeks = currentRevealedWeeks.filter((w: number) => w !== week)
    ...
    if (updateError) {
      return NextResponse.json({ 
        success: false, 
        error: 'Failed to hide picks: ' + updateError.message 
      })
    }

    return NextResponse.json({ 
      success: true,
      message: `Week ${week} picks have been hidden!`,
      data: {
        week: week,
    ...
    console.error('Hide picks error:', error)
    return NextResponse.json({ 
      success: false, 
      error: 'Internal server error hiding picks' 
    })
  }
}